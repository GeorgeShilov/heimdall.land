...
- for this project, the domain is `heimdall.land`
...
```text
https://web.archive.org/web/20160328162852/heimdall.land/
```
...
- Railway project: `heimdall.land`
- Railway service: `heimdall.land`
- Railway temporary domain: `https://heimdallland-production.up.railway.app`
- Custom domain: `heimdall.land`
...
```bash
railway init -n heimdall.land -w <workspace-id> --json
railway add --service heimdall.land --json
railway up --service heimdall.land --detach --verbose
railway domain --service heimdall.land --json
railway variable set SITE_URL=https://heimdall.land --service heimdall.land --environment production --skip-deploys
```

If `railway domain heimdall.land ...` returns `Unauthorized` even after `railway login`, use the Railway dashboard or Railway GraphQL API to create the custom domain. In this run, the CLI was intermittently authenticated for `status` but unauthorized for `domain`.
...
Railway gave these records for `heimdall.land`:
...
Zone > DNS > Edit
Zone > Zone > Read
Zone Resources: Include > Specific zone > heimdall.land
```
...
```bash
curl -s "https://cloudflare-dns.com/dns-query?name=heimdall.land&type=A" \
  -H "accept: application/dns-json"

curl -s "https://cloudflare-dns.com/dns-query?name=_railway-verify.heimdall.land&type=TXT" \
  -H "accept: application/dns-json"
```
